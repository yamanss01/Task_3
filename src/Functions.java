...
public final class Functions {
	...
	private static boolean parseBackground(String[] properties, WorldModel world, ImageStore imageStore) {
		if (properties.length == BGND_NUM_PROPERTIES) {
			...
			Background.setBackground(world, pt, new Background(id, ImageStoreAbstract.getImageList(imageStore, id)));
		}
	...
	private static boolean parseSapling(String[] properties, WorldModel world, ImageStore imageStore) {
		if (properties.length == SAPLING_NUM_PROPERTIES) {
			...
			Entity entity = new Entity(EntityKind.SAPLING, id, pt, ImageStoreAbstract.getImageList(imageStore, SAPLING_KEY), 0,
	...
	private static boolean parseDude(String[] properties, WorldModel world, ImageStore imageStore)
		...
		if (properties.length == DUDE_NUM_PROPERTIES) {
			...
			Entity entity = ActionAbstraction.createDudeNotFull(properties[DUDE_ID], pt,
					...
					ImageStoreAbstract.getImageList(imageStore, DUDE_KEY));
		 	Entity.tryAddEntity(world, entity);
		}
	...
	private static boolean parseFairy(String[] properties, WorldModel world, ImageStore imageStore)
		...
		if (properties.length == FAIRY_NUM_PROPERTIES) {
			...
			Entity entity = createFairy(properties[FAIRY_ID], pt, Integer.parseInt(properties[FAIRY_ACTION_PERIOD]),
					...
					ImageStoreAbstract.getImageList(imageStore, FAIRY_KEY));
			Entity.tryAddEntity(world, entity);
		}
	...
	private static boolean parseTree(String[] properties, WorldModel world, ImageStore imageStore) {
		if (properties.length == TREE_NUM_PROPERTIES) {
			...
			Entity entity = createTree(properties[TREE_ID], pt, Integer.parseInt(properties[TREE_Action_PERIOD]),
					...
					ImageStoreAbstract.getImageList(imageStore, TREE_KEY));
			Entity.tryAddEntity(world, entity);
		}
	...
	private static boolean parseHouse(String[] properties, WorldModel world, ImageStore imageStore) {
		if (properties.length == HOUSE_NUM_PROPERTIES) {
			...
			Entity entity = createHouse(properties[HOUSE_ID], pt, ImageStoreAbstract.getImageList(imageStore, HOUSE_KEY));
			Entity.tryAddEntity(world, entity);
		}
	...
	private static Entity createFairy(String id, Point position, int ActionInterfacePeriod, int animationPeriod,
			List<PImage> images) {
		return new Entity(EntityKind.FAIRY, id, position, images, 0, 0, ActionInterfacePeriod, animationPeriod, 0, 0);
	}
	...
	public static Entity createTree(String id, Point position, int ActionInterfacePeriod, int animationPeriod, int health,
			List<PImage> images) {
		return new Entity(EntityKind.TREE, id, position, images, 0, 0, ActionInterfacePeriod, animationPeriod, health, 0);
	}

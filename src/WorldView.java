...
public final class WorldView extends WorldViewAbstraction{
	...
	protected void shiftView(WorldView view, int colDelta, int rowDelta) {
	...
	protected void drawViewport(WorldView view) {
		drawBackground(view);
		drawEntities(view);
	}

	protected void drawBackground(WorldView view) {
	...
	protected void drawEntities(WorldView view) {
		for (Entity entity : view.world.entities) {
			...
			if (Viewport.contains(view.viewport, pos)) {
				...
				view.screen.image(ImageStoreAbstract.getCurrentImage(entity), viewPoint.x * view.tileWidth,
						viewPoint.y * view.tileHeight);
			}
		}
	}

	protected Point worldToViewport(Viewport viewport, int col, int row) {
